...
and `EagerInputStream` in Iceberg core. It fetches the file eagerly and serves
subsequent reads from memory, while working with any `InputFile` implementation.
...
- Optimize small-file Parquet reads by reducing redundant cloud requests across
...
![Timeline from first contributions to selection](timeline-origin.png)
...
![Three dependent requests per Parquet read, versus one eager fetch](parquet-requests.png)
...
wired into the Parquet read path ([#17284](https://github.com/apache/iceberg/pull/17284)).
...
![GSoC coding period timeline](timeline-gsoc.png)
...
![Compaction runtime comparison](compaction-benchmark-graph.png)
...
- **Pull requests** — contributing changes to Iceberg.
